...
import { HeatmapMatrix, type HeatmapRow } from "@/components/enterprise/heatmap-matrix";
import {
  SkillReadinessChart,
...
import type { DashboardOverview, DepartmentPerformance } from "@/types/dashboard";
    ...
    <div className="command-gradient min-h-full space-y-6 p-4 lg:p-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <ExecutiveMetricCard
          ...
          icon={Sparkles}
          href={`${ROUTES.ANALYTICS}?tab=team`}
        />
        <ExecutiveMetricCard
    ...
    <div className="command-gradient min-h-full space-y-6 p-4 lg:p-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <ExecutiveMetricCard
          ...
          icon={Shield}
          href={`${ROUTES.ANALYTICS}?tab=compliance`}
        />
        <ExecutiveMetricCard
...
export function ExecutiveDashboard({
  ...
  const heatmapRows = buildDepartmentHeatmap(data.departmentPerformance);
  const complianceRate =
    data.certificates.total > 0
      ? Math.round((data.certificates.active / data.certificates.total) * 100)
      : 0;
  const certificationCoverage = complianceRate;
    ...
    <div className="command-gradient min-h-full space-y-6 p-4 lg:p-6">
      ...
      <section aria-label="Executive KPIs">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <ExecutiveMetricCard
            ...
            subtitle={`${data.skillReadiness.verifiedCount} verified`}
            icon={Sparkles}
            href={`${ROUTES.ANALYTICS}?tab=employee`}
          />
          <ExecutiveMetricCard
            label="Compliance Rate"
            value={`${complianceRate}%`}
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <HeatmapMatrix rows={heatmapRows} title="Department Performance Heatmap" className="h-full" />
        </div>
        <AnalyticsPanel title="Workforce Snapshot" description="Compliance and learning signals">
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{certificationCoverage}% certification coverage across scope</p>
            <p>{data.certificates.expiringSoon} certificate(s) expiring within 30 days</p>
            <p>{data.learningProgress.completed} course completions · {data.assessments.passRate}% assessment pass rate</p>
          </div>
        </AnalyticsPanel>
      </section>
...
function buildDepartmentHeatmap(departments: DepartmentPerformance[]): HeatmapRow[] {
  return departments.map((d) => ({
    id: d.code,
    label: d.name,
    values: [
      { key: "skills", label: "Skills", value: d.skillReadiness },
      { key: "learning", label: "Learning", value: d.learningProgress },
      { key: "assess", label: "Assess", value: d.assessmentPassRate },
      { key: "headcount", label: "Team", value: Math.min(100, d.employees * 5) },
    ],
  }));
}
